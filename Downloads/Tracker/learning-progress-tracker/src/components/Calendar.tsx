...
import './Calendar.css'; // Importation du fichier CSS dédié
...
const Calendar: React.FC<Props> = ({ items, onDateSelect, selectedDate: externalSelectedDate, onAddItem }) => {
  ...
  const getDayContent = (day: CalendarDay) => {
    ...
    return (
      <div 
        ...
        data-date={getAdjustedDateStr(day.date)}
        data-activity={hasActiveTasks ? "true" : "false"}
        data-completed={hasCompletedTasks ? "true" : "false"}
      >
  ...
  return (
    <div className="relative space-y-4 w-full calendar-component">
      ...
      {/* Calendar Grid */}
      <div className="grid grid-cols-7 gap-1 w-full calendar-grid">
                ...
                <motion.button
                  ...
                  className={cn(
                    ...
                    selectedDay && isSelectedDate(day)
                      ...
                      : '',
                    day.isToday ? 'isToday' : ''  // Ajout d'une classe pour "aujourd'hui"
                  )}
                  data-today={day.isToday ? "true" : "false"}
                  data-current-week={weekIndex === Math.floor(new Date().getDate() / 7) ? "true" : "false"}
      ...
      {hoveredDay && (hoveredDay.activities.activeItems.length > 0 ||
        ...
        <div 
          className="fixed z-50 bg-white dark:bg-gray-800 shadow-xl rounded-lg p-4 w-72 border border-gray-200 dark:border-gray-700 transform -translate-x-full calendar-tooltip"
          style={{
