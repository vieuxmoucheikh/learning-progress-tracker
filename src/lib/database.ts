...
// Pomodoro Tasks Sync Functions
export async function fetchPomodoroTasks() {
  try {
    const { data, error } = await supabase
      .from('pomodoro_tasks')
      .select('*')
    ...
    console.error('Error in fetchPomodoroTasks:', error);
    return null;
  }
}

export async function savePomodoroTask(task: {
  id: string;
  text: string;
  completed: boolean;
  metrics: {
  ...
  try {
    ...
    if (!session) {
      console.error('No active session when saving pomodoro task');
      return null;
    }
    ...
    console.error('Error in savePomodoroTask:', error);
    return null;
  }
}

export async function deletePomodoroTask(taskId: string) {
  try {
    const { error } = await supabase
      .from('pomodoro_tasks')
      .delete()
...
export async function syncPomodoroTasks(localTasks: Array<{
  id: string;
  text: string;
  completed: boolean;
  metrics: {
  ...
  try {
    // Get remote tasks
    const remoteTasks = await fetchPomodoroTasks();
    
    if (!remoteTasks) {
      console.error('Failed to fetch remote tasks for syncing');
      return localTasks;
    }
    
    // Create a map of remote tasks by ID for easy lookup
    const remoteTasksMap = new Map(remoteTasks.map(task => [task.id, task]));
    
    // Create a map of local tasks by ID for easy lookup
    const localTasksMap = new Map(localTasks.map(task => [task.id, task]));
    
    // Tasks to be saved to the server (new or updated local tasks)
    const tasksToSave = [];
    
    // Merged tasks to return (will include both local and remote tasks)
    const mergedTasks = [];
    ...
    for (const localTask of localTasks) {
      ...
      // If task doesn't exist remotely or local version is newer, save it
      if (!remoteTask || new Date(localTask.updated_at || 0) > new Date(remoteTask.updated_at)) {
        tasksToSave.push(localTask);
        mergedTasks.push(localTask);
      } else {
        // Remote version is newer or same, use the remote version
        mergedTasks.push(remoteTask);
      }
      
      // Remove from remoteTasksMap to track which ones we've processed
      remoteTasksMap.delete(localTask.id);
    }
    
    // Add remaining remote tasks (ones that don't exist locally)
    ...
    for (const task of tasksToSave) {
      await savePomodoroTask(task);
    }
    
    return mergedTasks;
  } catch (error) {
    console.error('Error syncing pomodoro tasks:', error);
