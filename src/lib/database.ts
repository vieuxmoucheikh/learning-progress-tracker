...
export async function getPomodoroStats(): Promise<PomodoroStats> {
  try {
    ...
    const stats: PomodoroStats = {
      ...
      totalWorkMinutes: (todayPomodoros.filter(p => p.type === 'work').reduce((acc, p) => { if (p.end_time) { const duration = (new Date(p.end_time).getTime() - new Date(p.start_time).getTime()) / 1000 / 60; return acc + duration; } return acc; }, 0)).toFixed(2),
