...
export function YearlyActivityHeatmap({ 
  ...
  const getColorForCount = (count: number) => {
    ...
    if (count === 1) return 'bg-gray-200 hover:bg-gray-300 dark:bg-gray-200 dark:hover:bg-gray-300';
    if (count <= 3) return 'bg-gray-300 hover:bg-gray-400 dark:bg-gray-300 dark:hover:bg-gray-400';
  ...
  return (
    <div className="w-full bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl shadow-sm border border-gray-200/50 dark:border-white/10">
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-200/50 dark:border-white/10">
        ...
        <div className="text-lg font-semibold text-black dark:text-white">{selectedYear}</div>
        <Button
          variant="outline"
          size="sm"
          onClick={() => setSelectedYear(selectedYear + 1)}
      ...
      <div className="w-full p-3">
        <div className="w-full" style={{ fontSize: 'min(1.5vw, 11px)' }}>
            ...
            <div className="flex-1">
              <div className="grid grid-cols-[repeat(53,1fr)] gap-[2px]">
                {monthLabels.map((label, i) => (
                  <div
                    key={i}
                    className="text-black dark:text-white text-center font-medium"
                    style={{ 
          ...
          <div className="flex w-full">
            ...
            <div className="flex flex-col gap-[2px] pr-1.5">
              {DAYS.map((day) => (
                <div 
                  key={day} 
                  className="text-black dark:text-white flex items-center w-5 font-medium"
                  style={{ 
                    ...
                    {week.map((day, dayIndex) => (
                      <TooltipProvider key={day.date}>
                        <Tooltip>
                          ...
                          <TooltipContent 
                            side="top"
                            className="bg-white/90 dark:bg-gray-800/95 backdrop-blur-sm border border-gray-200/50 dark:border-white/10"
                          >
          ...
          {/* Legend */}
          <div className="flex items-center gap-1.5 mt-3 justify-end text-black dark:text-white">
