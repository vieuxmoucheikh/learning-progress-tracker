...
export function TabNavigation({ activeTab, onTabChange }: TabNavigationProps) {
  ...
  return (
    <div className="flex justify-center mb-8 border-b overflow-x-auto w-full bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
      <nav className="flex -mb-px max-w-2xl w-full min-w-max px-2" aria-label="Tabs">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={cn(
                "flex-shrink-0 text-sm font-medium py-4 px-4 border-b-2 flex items-center justify-center gap-2 transition-all",
